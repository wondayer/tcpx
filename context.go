...
// New a context.
// This is used for new a context for tcp server.
func NewTCPContext(conn net.Conn) *Context {
	return NewContext(conn)
}

// New a context.
// This is used for new a context for udp server.
func NewUDPContext(conn net.PacketConn, addr net.Addr) *Context {
	var online = CONTEXT_ONLINE
	return &Context{
		PacketConn:           conn,
		Addr:                 addr,
...
func (ctx *Context) Reply(messageID int32, src []byte) error {
	var buf []byte
	var e error
	buf, e = ctx.Packx.Pack(messageID, src)
	if e != nil {
...
func (ctx *Context) SendToUsername(username string, messageID int32, src []byte) error {
	if ctx.poolRef == nil {
...
// Make sure called `srv.WithBuiltInPool(true)`
func (ctx *Context) SendToConn(anotherCtx *Context, messageID int32, src []byte) error {
